 ...
 */
export const FileIcon: React.FC<FileIconProps> = ({ type, className = '', size = 48 }) => {
...
function getIconType(type: string): string {
  ...
  // 代码文件
  const codeTypes = ['js', 'jsx', 'ts', 'tsx', 'vue', 'py', 'java', 'cpp', 'c', 'h', 'cs', 'php', 'rb', 'go', 'rs', 'swift', 'kt', 'scala', 'sh', 'bash', 'json', 'xml', 'yaml', 'yml', 'toml', 'css', 'scss', 'sass', 'less', 'html', 'htm', 'r', 'dart', 'lua'];
  if (codeTypes.includes(lowerType)) return 'code';
  
  // 图片文件
  const imageTypes = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp', 'ico', 'tiff', 'tif', 'heic', 'heif'];
  if (imageTypes.includes(lowerType)) return 'image';
  
  // 视频文件
  const videoTypes = ['mp4', 'avi', 'mov', 'wmv', 'flv', 'mkv', 'webm', 'm4v', 'mpg', 'mpeg', '3gp', 'ogv'];
  if (videoTypes.includes(lowerType)) return 'video';
  
  // 音频文件
  const audioTypes = ['mp3', 'wav', 'flac', 'aac', 'ogg', 'wma', 'm4a', 'opus', 'ape', 'alac'];
  ...
  const spreadsheetTypes = ['xls', 'xlsx', 'csv', 'ods', 'numbers', 'tsv', 'xlsm', 'xlsb'];
  ...
  // 链接文件
  if (lowerType === 'url' || lowerType === 'lnk' || lowerType === 'link') return 'link';
  
  // 压缩文件
  const archiveTypes = ['zip', 'rar', '7z', 'tar', 'gz', 'bz2', 'xz', 'iso', 'dmg'];
  if (archiveTypes.includes(lowerType)) return 'archive';
  
  // 数据库文件
  const databaseTypes = ['db', 'sqlite', 'sqlite3', 'mdb', 'accdb', 'sql', 'dbf', 'mdf', 'ldf'];

...
import { Copy, Eye, FileText, Trash2, Link as LinkIcon, X, Check } from 'lucide-react';
import { toast } from 'sonner';
import dayjs from 'dayjs';
import { getMyShareList, cancelShares, getShareDetailById, getShareAccessRecords } from '@/api/share';
...
export function MySharesView() {
  ...
  const [currentShareForRecords, setCurrentShareForRecords] = useState<ShareItem | null>(null);
  ...
  const isAllSelected = shareList.length > 0 && selectedKeys.length === shareList.length;
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <LinkIcon className="h-16 w-16 text-muted-foreground mx-auto mb-4 opacity-50" />
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="w-28 text-center font-medium">查看次数</TableHead>
                <TableHead className="w-28 text-center font-medium">下载次数</TableHead>
                <TableHead className="w-32 text-center font-medium">分享权限</TableHead>
                <TableHead className="w-44 font-medium">创建时间</TableHead>
                <TableHead className="w-48 text-center font-medium">操作</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {shareList.map((share) => {
                ...
                return (
                  <TableRow key={share.id} className={cn('group', isSelected && 'bg-primary/5')}>
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setSelectedKeys(selectedKeys.filter((id) => id !== share.id));
                          }
                        }}
                      />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm truncate">{share.shareName}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      {share.isPermanent ? (
                        ...
                        <span
                          className={cn(
                            'text-sm',
                            isExpired(share.expireTime) ? 'text-destructive' : 'text-muted-foreground'
                          )}
                        >
                    ...
                    <TableCell className="text-center text-sm">
                      ...
                      {share.maxViewCount > 0 && (
                        <span className="text-muted-foreground text-xs"> / {share.maxViewCount}</span>
                      )}
                    </TableCell>
                    <TableCell className="text-center text-sm">
                      {share.downloadCount}
                      {share.maxDownloadCount > 0 && (
                        <span className="text-muted-foreground text-xs"> / {share.maxDownloadCount}</span>
                      )}
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {(!share.scope || share.scope.includes('download')) && (
                          <Badge variant="secondary" className="text-xs bg-green-100 text-green-700">下载</Badge>
                        )}
                      </div>
                    </TableCell>
            ...
            {shareDetailLoading ? (
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">分享名称</div>
                  <div className="text-sm text-foreground break-all">{currentShare.shareName}</div>
                </div>
                
                {getShareUrl(currentShare) && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">分享链接</div>
                    <div className="flex items-center gap-2">
                      <div className="text-sm text-foreground break-all flex-1">{getShareUrl(currentShare)}</div>
                      <Button 
                        size="icon" 
                        variant="outline" 
                        className="h-8 w-8 shrink-0"
                        onClick={() => handleCopyLink(getShareUrl(currentShare)!)}
                      >
                        {copiedDetailLink ? (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-sm text-foreground font-semibold text-primary tracking-wider">
                    ...
                    {currentShare.shareCode && (
                      <Button 
                        ...
                        variant="outline" 
                        className="h-8 w-8 shrink-0"
                        onClick={() => handleCopyCode(currentShare.shareCode!)}
                      >
                        {copiedDetailCode ? (
                          <Check className="h-4 w-4 text-green-600" />
                        ) : (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-sm text-foreground">
                    {currentShare.isPermanent ? '永久有效' : formatExpireTime(currentShare.expireTime)}
                  </div>
                </div>
                
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">分享权限</div>
                  <div className="text-sm text-foreground">{formatScopeText(currentShare.scope)}</div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-sm text-foreground">
                    ...
                    {currentShare.maxViewCount > 0 && ` / ${currentShare.maxViewCount}`}
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="text-sm text-foreground">
                    ...
                    {currentShare.maxDownloadCount > 0 && ` / ${currentShare.maxDownloadCount}`}
                  </div>
                </div>
                
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">创建时间</div>
                  <div className="text-sm text-foreground">{formatFileTime(currentShare.createdAt)}</div>
                </div>
              </div>
            )}
          </div>
      ...
      {/* 访问记录弹窗 */}
      <Dialog open={accessRecordsVisible} onOpenChange={setAccessRecordsVisible}>
        <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>访问记录 - {currentShareForRecords?.shareName || ''}</DialogTitle>
          </DialogHeader>
          <div className="space-y-6 px-6 pb-4">
            {accessRecordsLoading ? (
              <div className="py-8 text-center text-sm text-muted-foreground">加载中...</div>
            ) : accessRecords.length === 0 ? (
              <div className="py-8 text-center text-sm text-muted-foreground">暂无访问记录</div>
            ) : (
              <div className="overflow-auto max-h-[60vh]">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="font-medium text-muted-foreground">访问IP</TableHead>
                      <TableHead className="font-medium text-muted-foreground">访问地址</TableHead>
                      <TableHead className="font-medium text-muted-foreground">浏览器</TableHead>
                      <TableHead className="font-medium text-muted-foreground">操作系统</TableHead>
                      <TableHead className="font-medium text-muted-foreground">访问时间</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {accessRecords.map((record) => (
                      <TableRow key={record.id}>
                        <TableCell className="text-sm">{record.accessIp}</TableCell>
                        <TableCell className="text-sm">{record.accessAddress || '-'}</TableCell>
                        <TableCell>
                          <Badge variant="secondary" className="text-xs">{record.browser || '-'}</Badge>
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline" className="text-xs">{record.os || '-'}</Badge>
                        </TableCell>
                        <TableCell className="text-sm text-muted-foreground">
      ...
      <AlertDialog open={deleteDialogVisible} onOpenChange={setDeleteDialogVisible}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmCancelShare} className="bg-destructive hover:bg-destructive/90">
              确认
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      {/* 批量删除确认弹窗 */}
      <AlertDialog open={batchDeleteDialogVisible} onOpenChange={setBatchDeleteDialogVisible}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认批量取消</AlertDialogTitle>
            <AlertDialogDescription>
              确定要取消选中的 {selectedKeys.length} 个分享吗？取消后将无法恢复！
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction onClick={confirmBatchCancel} className="bg-destructive hover:bg-destructive/90">
              确认
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>

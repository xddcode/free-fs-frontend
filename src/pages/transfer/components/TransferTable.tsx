...
const getStatusColor = (status: TaskStatus): 'default' | 'secondary' | 'destructive' | 'outline' => {
  const colorMap: Record<TaskStatus, 'default' | 'secondary' | 'destructive' | 'outline'> = {
    idle: 'secondary',
...
export default function TransferTable({
  ...
  const canCancel = (status: TaskStatus) =>
    ['initialized', 'checking', 'uploading', 'paused', 'failed'].includes(status);
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {showActions && <TableHead className="w-[180px] text-center">操作</TableHead>}
            {showCompleteTime && <TableHead className="w-[180px]">完成时间</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {tasks.map((task) => {
              ...
              <TableRow key={task.taskId}>
                ...
                <TableCell>
                  ...
                  {task.status === 'checking' && (
                    <div className="flex items-center gap-2">
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-primary border-t-transparent" />
                      <span className="text-sm text-muted-foreground">校验文件...</span>
                    </div>
                  )}
                  ...
                  {task.status === 'uploading' && (
                    <div className="flex items-center gap-3">
                      <Progress value={displayData.progress} className="w-[200px]" />
                      <div className="flex flex-col gap-0.5 min-w-[100px]">
                        ...
                        {displayData.remainingTime > 0 && (
                          <span className="text-xs text-muted-foreground">
                            剩余 {formatRemainingTime(displayData.remainingTime)}
                          </span>
                        )}
                      </div>
                    </div>
